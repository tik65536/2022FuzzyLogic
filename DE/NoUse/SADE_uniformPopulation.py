        ...
        while (i<self.maxiter):
            ...
            if(updatecount==0):
                ...
                if(up-low<0):
                    up=1
                    low=0
                current_gen = np.random.default_rng().uniform(low,up,(self.pplSize,1))
                for k in range(1,self.R.shape[1]):
                    ...
                    if(up-low<0):
                        up=1
                        low=0
                    v = np.random.default_rng().uniform(low,up,(self.pplSize,1))
...
#                top = current_gen[top]
#                regenCount+=1
#                current_gen = np.random.uniform(0,1,(self.pplSize-1,self.R.shape[1]))
#                current_gen = np.vstack((top,current_gen))
#                for k in range(self.pplSize):
#                    scores[k] = self.fit(current_gen[k],k)
            print(f'SADE_uniform Run {i:3d} CurrentBest: {currentbest:10.8f}, OverallBest: {overallBest:10.8f}/{bestGen:3d}, Generation RunTime: {(end-start):10.8f}')
...
sade = SADE_FRE(maxiter=10000,R=R,b=b)
sade.run()
