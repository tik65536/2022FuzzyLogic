...
class JADE_FRE():
    def __init__(self, initSize=200,maxiter=100,error=1e-03,R=None,b=None):
        ...
        self.population = np.random.gamma(0.6,1,(self.pplSize,self.R.shape[1]))
    ...
    def run(self,beta=0.5,p=0.2):
        ...
        while (i<self.maxiter):
            ...
            if(updatecount==0):
                ...
                top = current_gen[top]
                k = np.mean(top)**2/np.var(top)
                theta = np.var(top)/np.mean(top)
                current_gen = np.random.gamma(k,theta,(self.pplSize-10,self.R.shape[1]))
            ...
            print(f'JADE_gamma Run {i:3d} CurrentBest: {currentbest:10.8f}, OverallBest: {overallBest:10.8f}/{bestGen:3d}, Generation RunTime: {(end-start):10.8f}')
