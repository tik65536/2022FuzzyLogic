...
class SADE_FRE():
    def __init__(self, initSize=200,maxiter=100,error=1e-03,R=None,b=None):
        ...
        self.population = np.random.default_rng().uniform(0,1,(self.pplSize,self.R.shape[1]))
    ...
    def run(self):
        ...
        lowBound = (overallBestConfig-0)/2
        bestGen = 0
        ...
        while (i<self.maxiter):
            ...
            if(updatecount==0):
                regenCount+=1
                mean=(upBound[0]+lowBound[0])/2
                current_gen = np.random.default_rng().normal(mean,0.2,(self.pplSize,1))
                for k in range(1,self.R.shape[1]):
                    mean=(upBound[k]+lowBound[k])/2
                    v = np.random.default_rng().normal(mean,0.2,(self.pplSize,1))
                    current_gen=np.hstack((current_gen,v))
                ...
                for k in range(self.pplSize):
                    ...
                    #if(s<scores[k]):
                    #    current_gen[k]=newGen[k]
                    #    scores[k]=s

            print(f'SADE Run {i:3d} CurrentBest: {currentbest:10.8f}, OverallBest: {overallBest:10.8f}/{bestGen:3d}, Generation RunTime: {(end-start):10.8f}')
